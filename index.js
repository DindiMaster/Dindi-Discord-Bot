...
client.on("message", async message => {
	...
	if(message.content.startsWith(`${prefix}play`)){
		...
		if(message.content.includes("https://www.youtube.com/playlist?list")){
			const playlist = await youtube.getPlaylist(url).catch(error => {
		...
		**Now Playing:** ${serverQueue.songs[0].title}
		...
		islooping = loop.get(message.guild.id) || true;
		if(!message.member.voice.channel) return message.channel.send("You need to be in a voice channel!");
	...
	if(message.content === (prefix + "quiz")){
		...
		if(message.content.toLowerCase(correctAnswer.toLowerCase()) && filter){
